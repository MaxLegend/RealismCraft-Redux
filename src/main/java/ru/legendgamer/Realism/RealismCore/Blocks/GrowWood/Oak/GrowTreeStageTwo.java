...
public class GrowTreeStageTwo extends BasicBlockWithCustomModel {
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
		...
		if(!world.isRemote) {
			...
			if (time2.time == 40) {
				...
				if(world.getBlockState(new BlockPos(pos.getX(), pos.getY() - maxHeightLeaves, pos.getZ())).getBlock() == Blocks.LOG){	
					...
					setBlock(new BlockPos(pos.getX(), pos.getY() + 2, pos.getZ() -1),world, RegBlocks.smallleaves.getDefaultState());
						...
						setBlock(new BlockPos(pos.getX() - 1, pos.getY(), pos.getZ()),world, RegBlocks.oakbranch.getDefaultState());
						setBlock(new BlockPos(pos.getX() , pos.getY(), pos.getZ() + 1),world, RegBlocks.oakbranch.getDefaultState());
						setBlock(new BlockPos(pos.getX() , pos.getY(), pos.getZ() - 1),world, RegBlocks.oakbranch.getDefaultState());
			...
			if (time2.time == 80) {
				setBlock(new BlockPos(pos.getX(), pos.getY(), pos.getZ()),world, Blocks.LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, BlockPlanks.EnumType.OAK));
			}		
		}
	}
